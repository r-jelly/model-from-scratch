...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        my_preds = rnn_classifier(x_seq)
        torch_output, _ = torch_rnn(x_seq)
        torch_preds = torch_fc(torch_output[:, -1, :])
        torch.testing.assert_close(my_preds, torch_preds)

        # Unbatched input with and without an explicit hidden state
        my_preds = rnn_classifier(x_seq[0], h_prev[0])
        torch_output, _ = torch_rnn(x_seq[0], h_prev[0].unsqueeze(0))
        torch_preds = torch_fc(torch_output[-1, :])
    ...
    rnn_classifier.zero_grad(set_to_none=True)
    ...
    with torch.no_grad():
        ...
        my_preds = lstm_classifier(x_seq)
        torch_output, _ = torch_lstm(x_seq)
        torch_preds = torch_fc(torch_output[:, -1, :])
        torch.testing.assert_close(my_preds, torch_preds)

        # Unbatched input with and without explicit hidden/cell states
        my_preds = lstm_classifier(x_seq[0], (h_prev[0], c_prev[0]))
        torch_output, _ = torch_lstm(
            x_seq[0],
    ...
    lstm_classifier.zero_grad(set_to_none=True)
